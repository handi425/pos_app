...
class DebtWithCustomer {
  DebtWithCustomer({required this.debt, required this.customer});
...
class DebtsDao extends DatabaseAccessor<PosDatabase> with _$DebtsDaoMixin {
  ...
  Stream<List<DebtWithCustomer>> watchOpenDebts() {
    final query =
        select(debtsTable).join([
            innerJoin(
              customersTable,
              customersTable.id.equalsExp(debtsTable.customerId),
            ),
          ])
          ..where(debtsTable.status.equalsValue(DebtStatus.open))
          ..orderBy([
            OrderingTerm(
              expression: debtsTable.createdAt,
    ...
    await transaction(() async {
      ...
      await (update(debtsTable)..where((tbl) => tbl.id.equals(debtId))).write(
        DebtsTableCompanion(
          ...
          status: Value(
            newRemaining == 0 ? DebtStatus.closed : DebtStatus.open,
          ),
        ),
      );
